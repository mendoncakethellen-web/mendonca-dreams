  ...
  useEffect(() => {
    const loadCart = async () => {
      ...
      if (user) {
        ...
        setCartItems(data || [])
        const cartTotal = (data || []).reduce((sum, item) => sum + (item.products?.price || 0) * item.quantity, 0)
        setTotal(cartTotal)
      }
      setLoading(false)
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg p-6 border h-fit" style={{ borderColor: "#e5e5e5" }}>
              ...
              <div className="border-t pt-4 mb-4" style={{ borderColor: "#e5e5e5" }}>
                <div className="flex justify-between font-semibold" style={{ color: "#001F3F" }}>
                  <span>Total</span>
                  <span>R$ {total.toFixed(2)}</span>
                </div>
              </div>
              <Button onClick={checkout} className="w-full text-white" style={{ background: "#001F3F" }}>
                Finalizar Compra
              </Button>
            </div>
          </div>
